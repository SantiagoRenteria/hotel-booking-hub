...
/// con routing key = <c>topico</c> ("reservas"). Mensaje persistente (sobrevive reinicio del broker). La
/// no-duplicación se garantiza en el consumidor (inbox idempotente), no aquí. En nube, el adaptador hermano es
/// Dapr→Service Bus (Épica 8), seleccionado por entorno tras el mismo puerto.
/// </summary>
    ...
    : IPublicadorEventos, IAsyncDisposable
{
    ...
    {
        // Si el broker no está disponible, la excepción se PROPAGA: el ProcesadorOutbox la captura, deja la fila
        // Pendiente y la re-reclama al vencer el lease (at-least-once del productor). NO se traga aquí (AC-E9.1.5).
    ...
    /// <summary>Conexión/canal perezosos y reutilizados; reconexión si el canal se cayó. Un solo hilo declara (gate).</summary>
