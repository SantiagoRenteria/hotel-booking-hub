...
- **Naming vs lista cerrada de sufijos → E-T** — `Sender`/`ISender` (contrato del mediator ADR-018) y el placeholder `PublicadorEventosLog` (sufijo `Log`) no están en la lista cerrada. Whitelistear los términos del mediator en `AGENTS.md` y renombrar/eliminar el placeholder cuando se construya el check de sufijos en CI. `[Comun/Mensajeria, Reservas.Infrastructure/Mensajeria]`
