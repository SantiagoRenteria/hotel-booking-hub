...
- **El invariante anti-overbooking se garantiza en el motor de datos** (índice `UNIQUE (HabitacionId, Noche)` sobre los slots de inventario, que arbitra el conflicto en el propio INSERT bajo READ COMMITTED → 409), nunca en lógica de aplicación; `SERIALIZABLE` se descartó por costo/contención (ver [[decisions-adr]] ADR-016). Ver [[concurrency-and-messaging]].
...
- **Reglas técnicas obligatorias:** `DateTimeOffset` (nunca `DateTime`), UUID v7 como identidad en las PKs (mapeadas no-clustered en SQL Server, con clustering key secuencial interna — ADR-017), Result Pattern para flujos esperados, Problem Details RFC 7807 en errores, concurrencia optimista con `rowversion`, versionado por URL `/api/v1/`. Detalle en [[stack-and-conventions]].
