...
| 4. Validación / anti-inyección | ✅ Código (existente) | 14 `AbstractValidator` (FluentValidation → 400); EF Core parametrizado (0 `FromSqlRaw`/`ExecuteSql` **en producción**; los 3 usos en tests son limpieza con SQL estático); `Regex` con `matchTimeout=200ms` (`ExpresionesValidacion.cs`) |
...
| 6. HTTPS/HSTS + CORS | ✅ Código (HSTS+CORS) · 📄 TLS en ingress | `ApiGateway/Program.cs`: `UseHsts` (no-dev, IncludeSubDomains + 1 año) tras `UseForwardedHeaders` (IsHttps real detrás del ingress); CORS allowlist explícita (nunca `AllowAnyOrigin`; `Cors:AllowedOrigins: []` **vacío por defecto = ningún origen cross-site**, se puebla por entorno); redirección/terminación TLS en el ingress (ACA, ADR-008) |
