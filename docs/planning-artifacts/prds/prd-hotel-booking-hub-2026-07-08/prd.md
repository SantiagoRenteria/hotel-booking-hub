...
- **FR-18** — Nunca coexisten dos reservas activas para la misma habitación con estancias solapadas, **aun bajo concurrencia**. La garantía reside en el **motor de datos** (unicidad de slots de inventario + transacción `SERIALIZABLE`), nunca en lógica de aplicación; el intento perdedor recibe **409 Conflict** (Problem Details).
...
- **NFR-2 · Concurrencia y consistencia** — Invariante anti-overbooking garantizado en el motor de datos (unicidad de slots + `SERIALIZABLE`); concurrencia optimista con `rowversion` en los agregados; consistencia eventual entre Bounded Contexts vía eventos + proyecciones. Ver [concurrency-and-messaging](../../../specs/spec-hotel-booking-hub/concurrency-and-messaging.md).
