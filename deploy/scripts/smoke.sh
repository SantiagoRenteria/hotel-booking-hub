...
# Cubre TODOS los endpoints alcanzables por el Gateway, con ambos roles y casos negativos:
...
#   3) Camino feliz (Agente): crear hotel -> habitación -> reserva -> cancelar (atajo).
#   4) Lecturas: disponibilidad (Viajero+Agente), listado y detalle de reservas.
#   5) Idempotencia: replay del Idempotency-Key -> 200 con la MISMA reserva.
#   6) Cancelación en DOS pasos: solicitud -> cancelaciones-pendientes -> resolución.
#   7) Negativos: 401 (sin token), 403 (Viajero en endpoint SoloAgente), 404 (reserva inexistente).
#   8) Verificar la propagación del evento al worker de Notificaciones (logs).
#
...
say "disponibilidad OK (200)"
ok A GET /api/v1/reservas >/dev/null && say "listado de reservas OK"
ok A GET "/api/v1/reservas/$RESERVA_ID" >/dev/null && say "detalle de reserva OK"

# ---- 5) Idempotencia (replay del Idempotency-Key -> 200 misma reserva) ----
...
# ---- 6) Cancelación en DOS pasos: solicitud -> pendientes -> resolución ----
...
# ---- 8) Verificar propagación del evento (worker consume del broker y notifica) ----
if [ -n "$RG" ] && command -v az >/dev/null 2>&1; then
  ...
  say "Evidencia del evento -> revisa: docker compose -f deploy/docker-compose.yml logs --tail 100 notificaciones"
fi

say "SMOKE OK — health + camino feliz + lecturas + idempotencia + cancelación en 2 pasos + negativos (401/403/404)."
