...
/// <summary>
/// Story 7.1 (AC-E7.1.3) — el span del consumidor del worker se correlaciona con la traza originante mediante
/// el <c>trace-id</c> de negocio transportado en el envelope, de modo que la actividad del worker es atribuible
/// a la misma petición. Verificación determinista con <c>ActivityListener</c> (transporte Dapr diferido: la
/// correlación se hace por trace-id; el traceparent completo lo aportaría el sidecar).
/// </summary>
public sealed class CorrelacionTrazaTests
{
    ...
    private static EventoIntegracion Evento(string? traceId) => new(
    ...
    private static DespachadorNotificaciones Crear() =>
        new(new ProcesadorOk(), new ContadorReintentosEnMemoria(), new DeadLetterNoop(), new OpcionesDespachador(3));
    ...
    public async Task El_span_del_consumidor_cuelga_de_la_misma_traza_que_el_trace_id_de_negocio()
    {
        ...
        await Crear().DespacharAsync(Evento(traceId), CancellationToken.None);
        ...
        Assert.Equal(traceId, span.TraceId.ToString());
    ...
    public async Task Acepta_un_traceparent_W3C_completo_y_conserva_el_trace_id()
    {
        ...
        // When: el worker despacha el evento.
        await Crear().DespacharAsync(Evento(traceparent), CancellationToken.None);
    ...
    public async Task Sin_trace_id_arranca_una_traza_raiz_sin_fallar()
    {
        ...
        // When: el worker despacha el evento.
        await Crear().DespacharAsync(Evento(traceId: null), CancellationToken.None);
