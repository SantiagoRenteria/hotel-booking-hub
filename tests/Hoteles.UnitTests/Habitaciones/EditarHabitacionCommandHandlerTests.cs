...
public sealed class EditarHabitacionCommandHandlerTests
{
    ...
    public async Task Happy_path_edita_y_devuelve_200()
    {
        ...
        var resultado = await new EditarHabitacionCommandHandler(repo, HotelesVisiblesFake.ConHotelVisible(), new ColaOutboxFake()).Handle(Comando(habitacion.Id), CancellationToken.None);
    ...
    public async Task Habitacion_inexistente_devuelve_404()
    {
        ...
        var resultado = await new EditarHabitacionCommandHandler(repo, HotelesVisiblesFake.ConHotelVisible(), new ColaOutboxFake()).Handle(Comando(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Conflicto_de_concurrencia_se_propaga()
    {
        ...
        await Assert.ThrowsAsync<ConflictoConcurrenciaException>(
            () => new EditarHabitacionCommandHandler(repo, HotelesVisiblesFake.ConHotelVisible(), new ColaOutboxFake()).Handle(Comando(habitacion.Id), CancellationToken.None));
    }
}
