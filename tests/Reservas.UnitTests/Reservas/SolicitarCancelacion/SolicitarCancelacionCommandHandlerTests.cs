...
/// Story 6.3: identidad server-side + aislamiento por agente (fail-closed 403 sin identidad; reserva ajena → 404).
/// </summary>
public sealed class SolicitarCancelacionCommandHandlerTests
{
    ...
    private SolicitarCancelacionCommandHandler CrearHandler(DateOnly hoy, string? agente = Dueno) =>
        new(_repo, new ContextoFake { AgenteActual = agente }, new CalculadorPenalidad(), _outbox, RelojFijo.EnFecha(hoy));

    private Reserva SembrarConfirmada(decimal precio = 500m, string? dueno = Dueno)
    {
        var reserva = Reserva.Crear(Guid.NewGuid(), Estancia.Crear(_entrada, _entrada.AddDays(2)), agenteEmail: dueno, precioTotal: precio);
    ...
    public async Task Evento_incluye_emails_del_huesped_principal_y_del_agente()
    {
        ...
        await CrearHandler(_entrada.AddDays(-40), agente: "carolina@agencia.com").Handle(Comando(reserva.Id), CancellationToken.None);
    ...
    public async Task Sin_identidad_devuelve_prohibido_sin_encolar()
    {
        ...
        var resultado = await CrearHandler(_entrada.AddDays(-40), agente: null).Handle(Comando(reserva.Id), CancellationToken.None);
    ...
    {
        // IDOR (AC-E6.3.4): un agente no puede solicitar la cancelación de la reserva de otro. 404 (no filtra existencia).
        var reserva = SembrarConfirmada(dueno: Dueno);

        var resultado = await CrearHandler(_entrada.AddDays(-40), agente: "otro@agencia.com").Handle(Comando(reserva.Id), CancellationToken.None);
